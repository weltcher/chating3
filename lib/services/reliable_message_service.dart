...
/// 可靠消息服务
///
/// 统一管理消息的发送和接收，确保消息可靠投递
///
/// 核心功能：
/// 1. 消息发送：通过消息队列 + 无限重试确保发送成功
/// 2. 消息持久化：发送前先存入本地数据库，App崩溃后可恢复
/// 3. 消息接收：通过去重服务确保不重复处理
/// 4. 离线同步：重连后自动同步离线消息
/// 5. 状态追踪：实时追踪消息状态变化（pending→sending→sent→delivered→read）
///
/// 可靠性保证：
/// - 无限重试：消息会一直重试直到发送成功（指数退避，最大60秒间隔）
/// - 持久化队列：pending_messages表确保App崩溃后消息不丢失
/// - ACK确认：等待服务器确认，超时自动重试
///
/// 适用于所有客户端组合：
/// - PC ↔ PC
/// - Mobile ↔ Mobile
